...
public class Projectile : MonoBehaviour {
    ...
    void CheckCollisions(float moveDistance)//checks if the projectile hits something before hitting it
    {
        ...
        RaycastHit hit;
        if (Physics.Raycast(ray, out hit, moveDistance + .2f, _collisionMask, QueryTriggerInteraction.Collide))//actual raycast, queryTriggerInteraction allows me to decide if i want it to collide with triggers to, wich is what i want in this case
        {
            OnHitObject(hit);
        }
    }
